...
router.get("/:username/shop", isLoggedIn, function(req, res) {
    Chest.find({}, function(err, foundChests) {
        let availableChests = foundChests.filter(chest => chest.isAvailable)
        res.render("profile/shop", { chests: availableChests, currencyList: Chest.currencyList });
    })
